...
const ATENDENTES_MOCK = ["Carlos", "Fernanda"]

interface FunilExpansaoProps {
  cards: CardLead[]
}

export function FunilExpansao({ cards }: FunilExpansaoProps) {
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden">
        <div className="flex gap-3 p-4 h-full items-start">
          {ETAPAS_EXPANSAO.map((etapa) => {
            ...
            return (
              <ColunaKanban key={etapa.id} titulo={etapa.label} cards={cardsColuna} />
            )
          })}
        </div>
      </div>
